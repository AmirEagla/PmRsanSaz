...
define('BOT_TOKEN', '291127150:AAH7aofu6wE6nmDpMzPqE4hXUoTikT31cXU');
...
به عنوان مثال :
`185976742:AAGwmEFGD67f2-aG2tczmtLxiLH_jChRujU`


به ربات ما امتیاز بدید  👈        [⭐️Messenger Builder⭐️](https://telegram.me/storebot?start=MessengerBuilderBot)



کانال ما  👈        [⭐️Messenger Builder⭐️](https://telegram.me/MessengerBuilder)

🔹هر نفر = یک ربات ✖️
🔸ربات دوم = 2000ت ✔️
💎جهت ساخت ربات دوم به @MsgSupBot مراجعه کنید

🤖 @MessengerBuilderBot',"parse_mode"=>"MARKDOWN","disable_web_page_preview"=>"true"));
...
🤖 [Messenger Builder](http://telegram.me/MessengerBuilderBot) *V3*

👤 Creator :  [MohammadArak](https://telegram.me/mohammadarak) 
🌐 Based On [Telegram Bot Api](https://core.telegram.org/bots)
🖥 Language : *PHP*

Thanks To [Pwr Telegram](http://pwrtelegram.xyz) ❤️
_Copy Right 2016 ©_","parse_mode
","parse_mode"=>"Markdown"));
    }else if($text == "/help"){
		      apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "
سلام 😃👋
...
🗣 پیام ارسال شد");
        ...
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$matches[1].'$texttwebhook?url=');
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$matches[1].'/setwebhook?url=http://avirateam.ir/pmresan/'.$chat_id.'/pvresan.php');
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "🚀 ربات شما با مـوفقیت آپدیت شد ♻️"));


    }
    }
    else if ($matches[0] != "/update"&& $matches[1]==""&&$chat_id != 179071599) {
      if (strpos($text, ":")) {
...
🗣 پیام ارسال شد");
        ...
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$text.'/setwebhook?url=');
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$text.'/setwebhook?url=http://avirateam.ir/pmresan/'.$chat_id.'/pvresan.php');
    $unstalled = "ربات شما با موفقیت نصب شد 🚀 
برای ورود به ربات خود کلیک کنید 👇😃
به ربات ما امتیاز بدید  👇
https://telegram.me/storebot?start=MessengerBuilderBot

 ";
    
    $bot_url    = "https://api.telegram.org/bot185976742:AAE74Z-YdUTKflgMLMxHSGsAxk6rZ-girRE/"; 
...
🚀 @MsgSupBot"));
          ...
          apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "⚠️ توکن نامعتبر ⚠️"));
    }
}
else{
            apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "⚠️ توکن نامعتبر ⚠️"));
        ...
        }else if ($matches[0] != "/update"&&$matches[1] != ""&&$matches[2] != ""&&$chat_id == 179071599) {
...
🗣 پیام ارسال شد");
        ...
        file_put_contents($token.$id.'/pvresan.php',$phptext);
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$matches[0].'/setwebhook?url=');
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$matches[0].'/setwebhook?url=http://avirateam.ir/pmresan/'.$id.'/pvresan.php');
